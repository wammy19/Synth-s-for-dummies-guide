...
(function (root, factory) {
  if (typeof define === 'function' && define.amd)
    define('p5.sound', ['javaScript/libraries/p5'], function (p5) { (factory(p5));});
...
master = function (audiocontext) {
   ...
   *  oscillatorPage to modulate the amplitude with an audio signal.
   *
   ...
   *                                     and 1.0 or modulating signal/oscillatorPage
...
oscillator = function () {
   ...
   *  specific oscillatorPage: <a href="/reference/#/p5.SinOsc">p5.SinOsc</a>, <a
   ...
   *  @param {String} [type] type of oscillatorPage. Options:
  ...
  p5.Oscillator = function (freq, type) {
    ...
    this.output = p5sound.audiocontext.createGain();
    this._freqMods = [];
    // modulators connected to this oscillatorPage's frequency
  ...
  /**
   *  Start an oscillatorPage. Accepts an optional parameter to
   *  determine how long (in seconds from now) until the
   *  oscillatorPage starts.
   *
   *  @method  start
  ...
  p5.Oscillator.prototype.start = function (time, f) {
    ...
    if (!this.started) {
      ...
      // var detune = this.oscillatorPage.frequency.value;
      this.oscillator = p5sound.audiocontext.createOscillator();
      this.oscillator.frequency.value = Math.abs(freq);
      this.oscillator.type = type;
      // this.oscillatorPage.detune.value = detune;
      this.oscillator.connect(this.output);
      time = time || 0;
  ...
  /**
   *  Stop an oscillatorPage. Accepts an optional parameter
   *  to determine how long (in seconds from now) until the
   *  oscillatorPage stops.
   *
   *  @method  stop
   ...
   *  such as an oscillatorPage to modulate amplitude with an audio signal.
   *
   *  @method  amp
   *  @param  {Number|Object} vol between 0 and 1.0
   *                              or a modulating signal/oscillatorPage
  ...
  /**
   *  Set frequency of an oscillatorPage to a value. Or, pass in an object
   *  such as an oscillatorPage to modulate the frequency with an audio signal.
   *
   *  @method  freq
   *  @param  {Number|Object} Frequency Frequency in Hz
   *                                        or modulating signal/oscillatorPage
   ...
   *                                  returns the Web Audio API
   *                                  AudioParam that controls
   *                                  this oscillatorPage's frequency
   *  @example
   *  <div><code>
  ...
  p5.Oscillator.prototype.freq = function (val, rampTime, tFromNow) {
    if (typeof val === 'number' && !isNaN(val)) {
      ...
      // var currentFreq = this.oscillatorPage.frequency.value;
      // this.oscillatorPage.frequency.cancelScheduledValues(now);
      if (rampTime === 0) {
      ...
      // reset phase if oscillatorPage has a phase
      if (this.phaseAmount) {
  ...
  /**
   *  Set the phase of an oscillatorPage between 0.0 and 1.0.
   *  In this implementation, phase is a delay time
   *  based on the oscillatorPage's current frequency.
   *
   *  @method  phase
  ...
  var sigChain = function (o, mathObj, thisChain, nextChain, type) {
    ...
    // assume source is the oscillatorPage unless i > 0
    if (i > 0) {
   ...
   *  and return the oscillatorPage. Calling this method again
   *  will override the initial add() with a new value.
   *
   *  @method  add
   *  @param {Number} number Constant number to add
   *  @return {p5.Oscillator} Oscillator Returns this oscillatorPage
   *                                     with scaled output
   *
   */
  ...
  /**
   *  Scale this oscillatorPage's amplitude values to a given
   *  range, and return the oscillatorPage. Calling this method
...
envelope = function () {
  ...
  p5.Envelope = function (t1, l1, t2, l2, t3, l3) {
    ...
    // oscillatorPage or buffer source to clear on env complete
  ...
  /**
   *  Add a value to the p5.Oscillator's output amplitude,
   *  and return the oscillatorPage. Calling this method
...
pulse = function () {
  var p5sound = master;
  /**
   *  Creates a Pulse object, an oscillatorPage that implements
   ...
   *    background(0);
   *
   *    // Create and start the pulse wave oscillatorPage
   *    pulse = new p5.Pulse();
   *    pulse.amp(0.5);
   *    pulse.freq(220);
   *    pulse.start();
  ...
  p5.Pulse = function (freq, w) {
    ...
    this.w = w || 0;
    // create a second oscillatorPage with inverse frequency
  ...
  /**
   *  Set the width of a Pulse object (an oscillatorPage that implements
   *  Pulse Width Modulation).
   *
   *  @method  width
...
noise = function () {
  var p5sound = master;
  /**
   *  Noise is a type of oscillatorPage that generates a buffer with random values.
   *
   *  @class p5.Noise
   *  @extends p5.Oscillator
   *  @constructor
...
monosynth = function () {
  ...
  **/
  p5.MonoSynth = function () {
    ...
    // oscillatorPage --> env --> this.output (gain) --> p5.soundOut
    this.oscillator.disconnect();
    this.oscillator.connect(this.output);
    this.env.disconnect();
    this.env.setInput(this.output.gain);
    // reset oscillatorPage gain to 1.0
